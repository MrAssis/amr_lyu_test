...
def contract_graph(G):
    NAMES = {}
    for n in G.nodes:
        if not G.nodes[n]["value"].cat == Rule_Frame:
            continue
        person = [nb for nb,edge in G[n].items()
                  if G.nodes[nb]["value"].le =="person" and  edge["role"] == ":ARG0" ]
    ...
    for n in G.nodes:
        if not G.nodes[n]["value"].le == "name":
            continue
        top = None
        topn = None
        names = []
        ...
        for nb,edge in G[n].items():
            if edge["role"] == ":name-of":
                top = G.nodes[nb]["value"]
                topn = nb
            if edge["role"].startswith(":op"):
                names.append((edge["role"][2:],G.nodes[nb]["value"]))
    ...
    for n in G.nodes:
        for n1,edge1 in G[n].items():
            for n2,edge2 in G[n].items():
                if str(n1) < str(n2) and G.nodes[n1]["value"] ==  G.nodes[n2]["value"] \
                    and edge1["role"] ==  edge2["role"] and G.nodes[n1]["value"].is_constant():
...
def graph_to_amr(G, sentence = None):
    ...
    uni = G.nodes[root]["value"]
    if uni.is_constant():
        G.nodes[root]["value"].cat = Rule_Concept
        G.nodes[root]["value"].le = G.nodes[root]["value"].le.strip(":").strip("/")
        if  G.nodes[root]["value"].le == "":
            G.nodes[root]["value"].le = "amr-unintelligible"
    s,visited,visited_edges =  sub_to_amr(G,{top,root},root,{(top,root,":top"),(root,top,":top-of")})
    for n in G.nodes:
        if not  n in visited:
            print ("node missing",n)
            print ("nearbour",G.nodes[n])
            for n in G.nodes:
                print ("nodes",n,G.nodes[n],G.nodes[n]["value"])
...
class AMRDecoder(object):
    ...
    def graph_to_quadruples(self,graph):
        def add_to_quadruples(h_v,d_v,r,r_inver):
            if is_core(r):
                quadruples.append([graph.nodes[h_v]['value'],graph.nodes[d_v]['value'],r,h_v,d_v])
            else:
                quadruples.append([graph.nodes[d_v]['value'],graph.nodes[h_v]['value'],r_inver,d_v,h_v])
        quadruples = []
        ...
        for n,d in graph.nodes(True):
            ...
            for nearb in graph[n]:
                if graph[n][nearb]["role"] in self.dicts["rel_dict"] :
                    ...
                    quadruples.append([graph.nodes[n]['value'],graph.nodes[nearb]['value'],r,n,nearb])
                elif  graph[n][nearb]["role"] == ":top-of":
                    quadruples.append([graph.nodes[nearb]['value'],graph.nodes[n]['value'],":top",nearb,n])
                    ...
                    if d["value"].le == "name":
                        ...
                        for nearb in graph[n]:
                            ...
                            if ":op" in r and "-of" not in r and  int(graph[n][nearb]["role"][3:]) not in names:
                                names.append([graph.nodes[nearb]["value"], int(graph[n][nearb]["role"][3:])])
                            if r == ":name-of":
                                wikied = False
...
# ::edge	border-01	ARG2	country	0.1.0	0.1.0.2
    def edges_jamr(self,graph,dep_only=False):
        ...
        for n,d in graph.nodes(True):
            ...
            if dep_only:
                for nearb in graph[n]:
                    if graph.nodes[nearb]["dep"] > d["dep"] and  graph.nodes[nearb]["value"].le != BOS_WORD:
                        head = graph.nodes[n]["value"].gold_str()
                        dep = graph.nodes[nearb]["value"].gold_str()
                        r = graph[n][nearb]["role"]
